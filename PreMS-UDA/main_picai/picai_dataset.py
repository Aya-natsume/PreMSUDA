...
class PICAITrainDataset(Dataset):
    def __init__(self, data_root, modality, n_slices, image_transform=None):
        ...
        assert self.modality in ('t2w', 'adc')
        ...
        self.slice_nums = self.patient_slices - self.n_slices + 1  # n_slices取法下每个病人的切片份数
...
class PICAIValidDataset(Dataset):
    def __init__(self, data_root, source_modality, target_modality, image_transform=None):
        self.source_modality = source_modality.lower()
        self.target_modality = target_modality.lower()
        self.image_transform = image_transform

        # self.data_file = h5py.File(
        #     os.path.join(data_root, "paired_%s_%s.h5" % (self.source_modality, self.target_modality)), 'r'
        # )

        self.data_file = h5py.File(
            os.path.join(data_root, "paired_%s_%s.h5" % (self.target_modality,self.source_modality)), 'r'
        )
        self.image_nums = self.data_file[self.source_modality].shape[0]  # 病人数量
        self.slice_nums = self.data_file[self.source_modality].shape[1]  # 每个病人的切片数量
        self.length = self.image_nums  # 验证集或测试集长度(病人数量)

    def __getitem__(self, index):
        source_image = self.data_file[self.source_modality][index]
        target_image = self.data_file[self.target_modality][index]
        if self.image_transform is not None:
            source_image = self.image_transform(source_image)
            target_image = self.image_transform(target_image)
        data_item = {self.source_modality: source_image, self.target_modality: target_image}
