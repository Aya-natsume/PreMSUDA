...
# own modules
def psnr(fake_image, real_image, peakval=1.):
    assert (real_image.max() <= 1.)
    assert (real_image.min() >= -1.)
    assert (fake_image.max() <= 1.)
    assert (fake_image.min() >= -1.)

    real_image = (real_image + 1.) / 2.
    fake_image = (fake_image + 1.) / 2.

    # mse = numpy.mean((real_image - fake_image) ** 2)
    mse = torch.mean((real_image - fake_image) ** 2)
...
def calc_multi_dice(result, gt, num_cls):
    ...
    for c in range(1, num_cls):
        ...
        dice_value = dc(_pred, _label)
        dice_list[c - 1] = float("{:.5f}".format(dice_value))
...
def calc_multi_assd(result, gt, num_cls):
    ...
    for c in range(1, num_cls):
        ...
        assd_value = assd(_pred, _label)
        assd_list[c - 1] = float("{:.5f}".format(assd_value))
...
def asd(result, reference, voxelspacing=None, connectivity=1):
    ...
    if str(result.device) == 'cpu' and str(reference.device) == 'cpu':
        sds = __surface_distances(result.numpy(), reference.numpy(), voxelspacing, connectivity)
    else:
        sds = __surface_distances(result.cpu().numpy(), reference.cpu().numpy(), voxelspacing, connectivity)
...
def __distinct_binary_object_correspondences(reference, result, connectivity=1):
    ...
    # find all overlaps from labelmap2 to labelmap1; collect one-to-one relationships and store all one-two-many for
    # later processing
    ...
    for l1id, slicer in enumerate(slicers):  # iterate over object in labelmap2 and their windows
        ...
        l2ids = numpy.unique(labelmap1[slicer][
                                 bobj])  # extract all unique object identifiers at the corresponding positions in
        # the reference (i.e. the mapping)
        l2ids = l2ids[0 != l2ids]  # remove background identifiers (=0)
        if 1 == len(
                l2ids):  # one-to-one mapping: if target label not already used, add to final list of
            # object-to-object mappings and mark target label as used
            l2id = l2ids[0]
