...
from amos_dataset import *

# AMOS: source_modality = "mri", target_modality = "ct"
# 超参数设置
torch.manual_seed(99)
device = torch.device("cuda:0")
data_root = "/home/chenxu/liudongliang/experiment_datas/amos"
source_modality = "mri"
target_modality = "ct"
image_shape = (216, 240, 320)
epochs = 5
input_slices = 1
class_nums = 14
sample_nums = 3
...
act_type = "swish"
act_type_D = "leaky"
os.makedirs("saved_models/original_version/PSNR", exist_ok=True)
os.makedirs("saved_models/original_version/DSC", exist_ok=True)
os.makedirs("saved_models/ema_version/DSC", exist_ok=True)
os.makedirs("saved_models/ema_version/PSNR", exist_ok=True)
os.makedirs("final_models/original_version/", exist_ok=True)
os.makedirs("final_models/ema_version/", exist_ok=True)
# 创建discriminator
...
train_dataloader = DataLoader(
    dataset=AMOSTrainDataset(
        data_root=os.path.join(data_root, "train"), modality=target_modality, n_slices=input_slices
    ), batch_size=batch_size, shuffle=True, num_workers=cpu_nums
)

# valid dataloader
valid_dataloader = DataLoader(
    dataset=AMOSValidDataset(data_root=os.path.join(data_root, "valid"), modality=target_modality),
    batch_size=1, shuffle=False, num_workers=cpu_nums
)

# test dataloader
test_dataloader = DataLoader(
    dataset=AMOSTestDataset(data_root=os.path.join(data_root, "test"), modality=target_modality),
    batch_size=1, shuffle=False, num_workers=cpu_nums
)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch_index, input_batch in enumerate(train_tqdm):
            real_slice = input_batch[target_modality].to(device)
            slice_label = input_batch["%s_seg" % target_modality].to(device)
            ...
            # 将loss值写进txt文件里面
            with open("%s_discriminator_loss.txt" % target_modality, "a") as file_D:
            ...
            # 将loss值写进txt文件里面
            with open("%s_generator_loss.txt" % target_modality, "a") as file_G:
        ...
        for batch_index, input_batch in enumerate(valid_tqdm):
            real_image = input_batch[target_modality].to(device)
            image_label = input_batch["%s_seg" % target_modality].to(device)
            ...
            # Print log
            valid_tqdm.set_postfix_str("target_PSNR:%.3f, target_DSC:%.3f%%" %
        ...
        for batch_index, input_batch in enumerate(test_tqdm):
            ...
            # Print log
            test_tqdm.set_postfix_str("target_PSNR:%.3f, target_DSC:%.3f%%" %
                                      (PSNR_value, torch.tensor(DSC_value).mean() * 100))
        ...
        if valid_PSNR_mean >= valid_best_PSNR and test_PSNR_mean >= test_best_PSNR:
            ...
            # 原始版本
            torch.save(GE.state_dict(), "saved_models/original_version/PSNR/GE_%s_best.pth" % target_modality)
            torch.save(GQ.state_dict(), "saved_models/original_version/PSNR/GQ_%s_best.pth" % target_modality)
            torch.save(GD.state_dict(), "saved_models/original_version/PSNR/GD_%s_best.pth" % target_modality)
            torch.save(SE.state_dict(), "saved_models/original_version/PSNR/SE_%s_best.pth" % target_modality)
            torch.save(SD.state_dict(), "saved_models/original_version/PSNR/SD_%s_best.pth" % target_modality)
            # EMA版本
            torch.save(GE_EMA.state_dict(), "saved_models/ema_version/PSNR/GE_%s_best.pth" % target_modality)
            torch.save(GQ_EMA.state_dict(), "saved_models/ema_version/PSNR/GQ_%s_best.pth" % target_modality)
            torch.save(GD_EMA.state_dict(), "saved_models/ema_version/PSNR/GD_%s_best.pth" % target_modality)
            torch.save(SE_EMA.state_dict(), "saved_models/ema_version/PSNR/SE_%s_best.pth" % target_modality)
            torch.save(SD_EMA.state_dict(), "saved_models/ema_version/PSNR/SD_%s_best.pth" % target_modality)
            print("Save my models with the best PSNR of target modality(Valid:%.3f-MAX, Test:%.3f-MAX)!" %
            ...
            torch.save(SE_EMA.state_dict(), "saved_models/ema_version/PSNR/SE_%s_best.pth" % target_modality)
            torch.save(SD_EMA.state_dict(), "saved_models/ema_version/PSNR/SD_%s_best.pth" % target_modality)
            print("Save my models with the best PSNR of target modality(Valid:%.3f, Test:%.3f-MAX)!" %
                  (valid_PSNR_mean, test_best_PSNR))
        elif valid_PSNR_mean >= valid_best_PSNR and test_PSNR_mean < test_best_PSNR:
            valid_best_PSNR = valid_PSNR_mean
            print("Didn't save my models with the best PSNR of target modality(Valid:%.3f-MAX, Test:%.3f)!" %
                  (valid_best_PSNR, test_PSNR_mean))
        else:
            print("Didn't save my models with the PSNR of target modality(Valid:%.3f, Test:%.3f)!" %
        ...
        if valid_DSC_mean >= valid_best_DSC and test_DSC_mean >= test_best_DSC:
            ...
            # 原始版本
            torch.save(GE.state_dict(), "saved_models/original_version/DSC/GE_%s_best.pth" % target_modality)
            torch.save(GQ.state_dict(), "saved_models/original_version/DSC/GQ_%s_best.pth" % target_modality)
            torch.save(GD.state_dict(), "saved_models/original_version/DSC/GD_%s_best.pth" % target_modality)
            torch.save(SE.state_dict(), "saved_models/original_version/DSC/SE_%s_best.pth" % target_modality)
            torch.save(SD.state_dict(), "saved_models/original_version/DSC/SD_%s_best.pth" % target_modality)
            # EMA版本
            torch.save(GE_EMA.state_dict(), "saved_models/ema_version/DSC/GE_%s_best.pth" % target_modality)
            torch.save(GQ_EMA.state_dict(), "saved_models/ema_version/DSC/GQ_%s_best.pth" % target_modality)
            torch.save(GD_EMA.state_dict(), "saved_models/ema_version/DSC/GD_%s_best.pth" % target_modality)
            torch.save(SE_EMA.state_dict(), "saved_models/ema_version/DSC/SE_%s_best.pth" % target_modality)
            torch.save(SD_EMA.state_dict(), "saved_models/ema_version/DSC/SD_%s_best.pth" % target_modality)
            print("Save my models with the best DSC of target modality(Valid:%.3f%%-MAX, Test:%.3f%%-MAX)!" %
            ...
            torch.save(SE_EMA.state_dict(), "saved_models/ema_version/DSC/SE_%s_best.pth" % target_modality)
            torch.save(SD_EMA.state_dict(), "saved_models/ema_version/DSC/SD_%s_best.pth" % target_modality)
            print("Save my models with the best DSC of target modality(Valid:%.3f%%, Test:%.3f%%-MAX)!" %
                  (valid_DSC_mean * 100, test_best_DSC * 100))
        elif valid_DSC_mean >= valid_best_DSC and test_DSC_mean < test_best_DSC:
            valid_best_DSC = valid_DSC_mean
            print("Didn't save my models with the best DSC of target modality(Valid:%.3f%%-MAX, Test:%.3f%%)!" %
                  (valid_best_DSC * 100, test_DSC_mean * 100))
        else:
            print("Didn't save my models with the DSC of target modality(Valid:%.3f%%, Test:%.3f%%)!" %
                  (valid_DSC_mean * 100, test_DSC_mean * 100))

        with open("%s_psnr_metrics.txt" % target_modality, "a") as file_valid:
            file_valid.write("target_PSNR:")
            for z in valid_PSNR_list:
                file_valid.write("{:.3f}\t".format(z))
            file_valid.write("\ntarget_PSNR_mean:%.3f" % valid_PSNR_mean)
            file_valid.write("\ntarget_PSNR_std:%.3f" % valid_PSNR_std)
            file_valid.write("\n\n")
        file_valid.close()
        ...
        with open("%s_dsc_metrics.txt" % target_modality, "a") as file_valid:
            file_valid.write("target_DSC:\n")
            for z in valid_DSC_list:
                file_valid.write("{}\n".format(z))
            file_valid.write("\ntarget_organ_DSC_mean(%):")
            for z in organ_DSC_mean:
                file_valid.write("{:.3f}\t".format(z * 100))
            file_valid.write("\ntarget_organ_DSC_std:")
            for z in organ_DSC_std:
                file_valid.write("{:.3f}\t".format(z))
            file_valid.write("\ntarget_DSC_mean(%%):%.3f" % (valid_DSC_mean * 100))
            file_valid.write("\ntarget_DSC_std:%.3f" % valid_DSC_std)
            file_valid.write("\n\n\n")
        file_valid.close()

    # 保存最后的模型
    # 原始版本
    torch.save(GE.state_dict(), "final_models/original_version/GE_%s_final.pth" % target_modality)
    torch.save(GQ.state_dict(), "final_models/original_version/GQ_%s_final.pth" % target_modality)
    torch.save(GD.state_dict(), "final_models/original_version/GD_%s_final.pth" % target_modality)
    torch.save(SE.state_dict(), "final_models/original_version/SE_%s_final.pth" % target_modality)
    torch.save(SD.state_dict(), "final_models/original_version/SD_%s_final.pth" % target_modality)
    # EMA版本
    torch.save(GE_EMA.state_dict(), "final_models/ema_version/GE_%s_final.pth" % target_modality)
    torch.save(GQ_EMA.state_dict(), "final_models/ema_version/GQ_%s_final.pth" % target_modality)
    torch.save(GD_EMA.state_dict(), "final_models/ema_version/GD_%s_final.pth" % target_modality)
    torch.save(SE_EMA.state_dict(), "final_models/ema_version/SE_%s_final.pth" % target_modality)
    torch.save(SD_EMA.state_dict(), "final_models/ema_version/SD_%s_final.pth" % target_modality)
    print("训练结束")
