...
test_dataloader = DataLoader(
    dataset=AMOSTestDataset(data_root=os.path.join(data_root, "valid"), modality=source_modality),
    batch_size=1, shuffle=False, num_workers=cpu_nums
)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch_index, input_batch in enumerate(train_tqdm):
            ...
            slice_label = input_batch["%s_seg" % source_modality].to(device)
        ...
        for batch_index, input_batch in enumerate(valid_tqdm):
            ...
            image_label = input_batch["%s_seg" % source_modality].to(device)
