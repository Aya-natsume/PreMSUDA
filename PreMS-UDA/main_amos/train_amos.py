...
data_root = "/home/chenxu/zxy/dataset/amos"
# source_modality = "mri"
# target_modality = "ct"
...
train_dataloader_S = DataLoader(
    dataset=AMOSTrainDataset(data_root=os.path.join(data_root, "train"), n_slices=n_slices, modality=source_modality),
    batch_size=batch_size, shuffle=True, num_workers=cpu_nums
)
train_dataloader_T = DataLoader(
    dataset=AMOSTrainDataset(data_root=os.path.join(data_root, "train"), n_slices=n_slices, modality=target_modality),
    batch_size=batch_size, shuffle=True, num_workers=cpu_nums
)

# valid dataloader
# valid_dataloader_S = DataLoader(
#     dataset=AMOSValidDataset(data_root=os.path.join(data_root, "valid"), modality=source_modality),
#     batch_size=1, shuffle=False, num_workers=cpu_nums
# )
valid_dataloader_T = DataLoader(
...
def main():
    ...
    # valid_length_S, valid_length_T = len(valid_dataloader_S), len(valid_dataloader_T)
    # valid_length = valid_length_S if valid_length_S <= valid_length_T else valid_length_T
    ...
    for epoch in range(epochs):
        ...
        for batch_index, (source_batch, target_batch) in enumerate(train_tqdm):
            ...
            S_image_label = source_batch["%s_seg" % source_modality].to(device)
        ...
        # valid_tqdm = tqdm(zip(valid_dataloader_S, valid_dataloader_T), total=valid_length, ncols=200)
        valid_tqdm = tqdm(valid_dataloader_T, total=len(valid_dataloader_T), ncols=200)
        valid_tqdm.set_description_str("[Valid]-[Epoch:%d/%d]" % (epoch + 1, epochs))
        # for batch_index, (source_batch, target_batch) in enumerate(valid_tqdm):
        for batch_index, target_batch in enumerate(valid_tqdm):
            # real_image_S = source_batch[source_modality].to(device)
            # S_image_label = source_batch["%s_seg" % source_modality].to(device)
            real_image_T = target_batch[target_modality].to(device)
            T_image_label = target_batch["%s_seg" % target_modality].to(device)
            ...
            # 计算DSC
            # source_DSC = calc_multi_dice(real_seg_S.squeeze(1), S_image_label.squeeze(0), num_cls=class_nums)
            # DSC_list_S.append(source_DSC)
            target_DSC = calc_multi_dice(real_seg_T.squeeze(1), T_image_label.squeeze(0), num_cls=class_nums)
            DSC_list_T.append(target_DSC)

            # 计算ASSD
            # source_ASSD = calc_multi_assd(real_seg_S.squeeze(1), S_image_label.squeeze(0), num_cls=class_nums)
            # ASSD_list_S.append(source_ASSD)
            target_ASSD = calc_multi_assd(real_seg_T.squeeze(1), T_image_label.squeeze(0), num_cls=class_nums)
            ...
            #         # save_image(S_image_label[:, i:i + 1].cpu(), class_nums=class_nums, is_seg=True,
            #         #            save_path="display_images/%s_label/patient_%d-slice_%d.png" %
            #         #                      (source_modality, batch_index + 1, i + 1))
            #         save_image(real_image_T[:, i:i + 1].cpu(), class_nums=class_nums, is_seg=False,
            #                    save_path="display_images/image/real_%s/patient_%d-slice_%d.png" %
            #                              (target_modality, batch_index + 1, i + 1))
            #         save_image(T_image_label[:, i:i + 1].cpu(), class_nums=class_nums, is_seg=True,
            #                    save_path="display_images/%s_label/patient_%d-slice_%d.png" %
            #                              (target_modality, batch_index + 1, i + 1))
